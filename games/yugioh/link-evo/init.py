...
#
# todos
#   * handle when symlinks cant be created or the dir does not exist
import os
import re
import shutil
import sys
...
def main() -> None:
    ...
    pattern = pattern.replace("\\", "\\\\").replace("(", r"\(").replace(")", r"\)")  # TODO: bad practice

    for path, _, _ in os.walk(dir_steam_userdata, followlinks=True):
